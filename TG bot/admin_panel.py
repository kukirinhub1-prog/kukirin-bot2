...
# Графічна панель адміністратора (Tkinter) з лічильником, видаленням та експортом
class AdminApp:
    ...
    def complete_order(self):
        ...
        if messagebox.askyesno("Підтвердження", "Позначити це замовлення як виконане і видалити зі списку?"):
            ...
            self.populate_table()
            messagebox.showinfo("Успіх", "Замовлення виконано та прибрано з бази!")
    ...
    def export_excel(self):
        ...
        if not orders:
            ...
            return
        file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv"), ("All files", "*.*")], title="Зберегти як")
        if file_path:
            try:
    ...
    def clear_all(self):
        ...
        if not orders:
            ...
            return
        if messagebox.askyesno("УВАГА!", "Ви впевнені, що хочете видалити ВСІ замовлення? Цю дію неможливо скасувати!"):
